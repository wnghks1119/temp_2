...
class SQLHelper {
  ...
  static Future<sql.Database> db() async {
    return await sql.openDatabase("database_name_modify.db", version: 1,
  ...
  static Future<List<Map<String, dynamic>>> getAllData() async {
    final db = await SQLHelper.db();
    return db.rawQuery('SELECT * FROM data ORDER BY year asc, month asc, day asc');
  ...
  static Future<List<Map<String, dynamic>>> getMonthData(int year, int month) async {
    ...
    //return db.query('data', where:'month = ? and year = ?', whereArgs: [month, year]);

    return db.rawQuery('SELECT * FROM data WHERE year = $year and month = $month ORDER BY year asc, day asc');
  ...
  static Future<int> updateDescData(int year, int month, int day, String? desc) async {
    final db = await SQLHelper.db();
    /*
    final data = {
      'Desc': desc
